...
function Swatch({ scheme }: { scheme: string | null }) {
  ...
  if (!sw) {
    return <span className="size-4 shrink-0 rounded-sm bg-muted ring-1 ring-border" />
  }
  return (
    <span
      className="flex h-4 w-5.5 shrink-0 items-center justify-center rounded-sm font-mono text-[9px] ring-1 ring-border"
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <Select
        items={options}
        value={value ?? INHERIT}
        onValueChange={(next: string | null) => onChange(!next || next === INHERIT ? null : next)}
      >
        {/* h-9 to match Input; same modifier chain so it beats the h-8 default. */}
        <SelectTrigger id="host-term-scheme" className="w-full data-[size=default]:h-9">
          <SelectValue>
            {(selected: string) => (
              <>
