  ...
  const renameRemote = async (f: FileEntryType) => {
    ...
    try {
      await sftpService.rename(
        hostId,
        f.path,
        joinPath(parentPath(f.path), name),
      );
      await loadRemote(hostId, remotePath);
    } catch (e) {
      setStatus({ msg: `Rename failed: ${e}`, kind: 'error' });
  ...
  return (
    <div className="flex min-w-0 flex-1 flex-col bg-[var(--bg)]">
      <div className="flex items-center gap-[14px] border-b border-[var(--border)] px-6 py-4">
        ...
        {status && (
          <span
            className="max-w-[460px] overflow-hidden text-[12px] text-ellipsis whitespace-nowrap"
            style={{
              ...mono,
...
function HostSelect({
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        {selected ? (
          <>
            ...
            <span
              className="overflow-hidden text-[11px] text-ellipsis whitespace-nowrap text-[var(--text-faint)]"
              style={mono}
            >
        ...
        <svg
          ...
          className="ml-auto shrink-0 text-[var(--text-faint)]"
        >
          <path d="M6 9l6 6 6-6" />
        </svg>
      </button>
      {open && (
        <div
          className="absolute top-[calc(100%+6px)] left-0 z-30 max-h-[340px] min-w-[300px] animate-[rise_0.14s_ease_both] overflow-y-auto rounded-[11px] border border-[var(--border-2)] bg-[linear-gradient(160deg,var(--bg-card-top),var(--bg-card))] p-[6px]"
          style={menuShadow}
        >
          {hosts.length === 0 && (
            <div className="p-3 text-[12px] text-[var(--text-faint)]">
              No saved hosts
            </div>
          )}
          {hosts.map((h) => (
            <button
              key={h.id}
              className={`flex w-full cursor-pointer items-center gap-[9px] rounded-lg px-[10px] py-[9px] text-left ${h.id === value ? 'bg-[var(--brand-soft-2)]' : 'hover:bg-foreground/5 bg-transparent'}`}
              onClick={() => {
                onChange(h.id);
                setOpen(false);
              }}
            >
              ...
              <span
                className="ml-auto text-[11px] text-[var(--text-faint)]"
                style={mono}
              >
...
function FileIcon({ link }: { link?: boolean }) {
  return (
    <svg
      ...
      strokeWidth="1.7"
      strokeLinejoin="round"
      style={{ color: link ? '#5b9dff' : '#8b95a6' }}
    >
    ...
    <div className="flex min-w-0 flex-1 flex-col border-r border-[var(--border)] last:border-r-0">
      <div className="box-border flex h-[52px] items-center gap-2 border-b border-[var(--border)] bg-[var(--bg-panel)] px-[18px]">
        <span
          ...
          style={{ color: local ? 'var(--text-dim)' : 'var(--brand)' }}
        >
          {local ? (
            <svg
              width="15"
        ...
        <span
          className="flex-1 overflow-hidden text-[12px] text-ellipsis whitespace-nowrap text-[var(--text-faint)]"
          style={mono}
        >
          {path}
        </span>
        {path && (
          <button
            className="bg-foreground/[0.04] h-[26px] w-[26px] shrink-0 cursor-pointer rounded-[7px] border border-[var(--border-2)] text-[var(--text-muted)] hover:border-[var(--brand-border)] hover:text-[var(--brand)]"
            onClick={onUp}
            title="Up one level"
          >
            ↑
      ...
      <div className="flex-1 overflow-y-auto">
        {(empty || error || busy) && (
          <div className="flex h-full min-h-[160px] items-center justify-center p-5 text-center text-[13px] text-[var(--text-faint)]">
            {busy ? (
              ...
              <span
                style={{ color: error ? 'var(--red)' : 'var(--text-faint)' }}
              >
                {error ?? empty}
              </span>
            )}
        ...
        {!empty &&
          ...
          visible.map((f) => (
            <div
              key={f.path}
              className={`group border-foreground/[0.03] flex items-center gap-3 border-b px-[18px] py-[9px] ${selected === f.path ? 'bg-[var(--brand-soft)]' : 'hover:bg-foreground/[0.03]'}`}
              style={
                selected === f.path
            ...
            >
              ...
              <span
                className="flex-1 overflow-hidden text-[12.5px] text-ellipsis whitespace-nowrap"
                style={{
                  ...mono,
                  color: f.isDir ? '#e8d3a8' : 'var(--text-bright)',
              ...
              <span
                className="w-[70px] text-right text-[11px] text-[var(--text-faint)]"
                style={mono}
              >
                {fmtSize(f.size, f.isDir)}
              </span>
              <span
                className="w-[96px] text-right text-[11px] text-[var(--text-faint)]"
                style={mono}
              >
                {fmtDate(f.modified)}
              </span>
...
function PaneMenu({
  ...
  const item =
    'flex w-full cursor-pointer items-center gap-2 rounded-[7px] bg-transparent px-[10px] py-2 text-left text-[12.5px] text-[var(--text)] hover:bg-[var(--brand-soft)] hover:text-[var(--brand)] disabled:cursor-default disabled:opacity-40 disabled:hover:bg-transparent disabled:hover:text-[var(--text)]';

  return (
    <div className="relative shrink-0" ref={ref}>
      <button
        className="bg-foreground/[0.04] h-[26px] w-[26px] shrink-0 cursor-pointer rounded-[7px] border border-[var(--border-2)] text-[var(--text-muted)] hover:border-[var(--brand-border)] hover:text-[var(--brand)] disabled:cursor-default disabled:opacity-40"
        onClick={() => setOpen((o) => !o)}
        disabled={disabled}
        title="Actions"
      >
      ...
      {open && (
        <div
          className="absolute top-[calc(100%+6px)] right-0 z-30 min-w-[190px] animate-[rise_0.14s_ease_both] rounded-[11px] border border-[var(--border-2)] bg-[linear-gradient(160deg,var(--bg-card-top),var(--bg-card))] p-[6px]"
          style={menuShadow}
        >
